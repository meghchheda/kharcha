...
# On most hosting platforms (Render, Railway, Fly.io, etc.) you can attach a
# persistent disk and point DB_PATH at it via an environment variable, e.g.
# DB_PATH=/data/expenses.db. Locally it just creates the file next to app.py.
...
# ---------------------------------------------------------------------------
# API: summary (totals by category) — handy for the dashboard
# ---------------------------------------------------------------------------
...
# Make sure the table exists as soon as the module is imported (works for
# both `python app.py` and gunicorn/WSGI runners).
init_db()
